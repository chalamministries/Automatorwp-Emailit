 ...
 *
 * @package     AutomatorWP\Integrations\MailPoet\Triggers\Submit_Form
...
class AutomatorWP_MailPoet_Anonymous_Submit_Form extends AutomatorWP_Integration_Trigger {

    public $integration = 'mailpoet';
    public $trigger = 'mailpoet_anonymous_submit_form';
    ...
    public function register() {
        ...
        automatorwp_register_trigger( $this->trigger, array(
            ...
            'anonymous'         => true,
            'label'             => __( 'Guest submits a form', 'automatorwp-mailpoet' ),
            'select_option'     => __( 'Guest submits <strong>a form</strong>', 'automatorwp-mailpoet' ),
            /* translators: %1$s: Form. %2$s: Number of times. */
            'edit_label'        => sprintf( __( 'Guest submits %1$s %2$s time(s)', 'automatorwp-mailpoet' ), '{post}', '{times}' ),
            /* translators: %1$s: Form. */
            'log_label'         => sprintf( __( 'Guest submits %1$s', 'automatorwp-mailpoet' ), '{post}' ),
            ...
            'options'           => array(
                'post' => automatorwp_utilities_ajax_selector_option( array(
                    'field'             => 'post',
                    'name'              => __( 'Form:', 'automatorwp-ameliabooking' ),
                    'option_none_value' => 'any',
                    'option_none_label' => __( 'any form', 'automatorwp-ameliabooking' ),
                    'action_cb'         => 'automatorwp_mailpoet_get_forms',
                    'options_cb'        => 'automatorwp_mailpoet_options_cb_form',
                    'default'           => 'any'
                ) ), 
            ...
            'tags' => array_merge(
                array(
                    'form_field:FIELD_NAME' => array(
                        'label'     => __( 'Form field value', 'automatorwp-mailpoet' ),
                        'type'      => 'text',
                        'preview'   => __( 'Form field value, replace "FIELD_NAME" by the field name', 'automatorwp-mailpoet' ),
                    ),
                ),
     ...
     */
    public function listener( $data, $segmentIds, $form ) {
        ...
        $form_fields = automatorwp_mailpoet_get_form_fields_values( $data );
    ...
    public function log_fields( $log_fields, $log, $object ) {
        ...
        $log_fields['form_fields'] = array(
            'name' => __( 'Fields Submitted', 'automatorwp-mailpoet' ),
            'desc' => __( 'Information about the fields values sent on this form submission.', 'automatorwp-mailpoet' ),
            'type' => 'text',
        );
...
new AutomatorWP_MailPoet_Anonymous_Submit_Form();